        ...
        children: [
          ...
          },
          {
            ...
            fields: {
              ...
              blockType: 'code',
              code: "export async function handleWebhook(request) {\n    const event = request.headers.get('x-gotocosmic-event');\n    const signature = request.headers.get('x-gotocosmic-signature');\n    const payload = await request.json();\n\n    if (!event || !signature) {\n        return new Response('Invalid webhook', { status: 400 });\n    }\n\n    console.log('Received event', event, payload.resourceID);\n    return Response.json({ ok: true });\n}\n",
              language: 'javascript',
            },
            format: '',
            version: 2,
